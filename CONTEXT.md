...
**Human takeover**:
User interaction with a retained subagent marks it as taken over, pins its record, and detaches any foreground wait. Subsequent terminal output stays in the in-memory child session for explicit message selection; completion itself neither writes a parent inbox entry nor requests a wake. Each explicit selection creates a separate persisted delivery without changing prior entries. The editor owns instructions and continuation; the delivery selector owns message selection. Child sessions and unselected output do not survive runtime reload or process exit.
