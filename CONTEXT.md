...
- The list stays hidden while no subagent records exist. Once visible, Main is a sticky row followed by at most six subagents and shows explicit running, queued, and total counts, including zero running or queued values. Child-interaction failures replace that summary with a local `Blocked:` reason; they do not notify in Main's transcript area.
