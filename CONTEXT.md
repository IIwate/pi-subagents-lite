...
**Invocation snapshot**:
The model, parent model, thinking selection, scoped-model state, and background delivery mode locked after an Agent call passes authorization. Running and queued agents use this snapshot; later configuration, parent-model, or scope changes affect only future calls.
_Avoid_: Queue revalidation, live assignment

**Background result delivery**:
A completed background result is persisted with the parent session ID and Agent-call origin entry before delivery. Every successfully persisted Auto completion rechecks eligible Auto pending state, but concurrent wake requests are coalesced. A successfully persisted Auto completion arriving during a failed parent turn provides one later automatic wake opportunity; the failed delivery alone never retries itself automatically. Next-turn completions never trigger Auto delivery. Automatic delivery is allowed only while the origin entry remains on the active branch. Explicit session reload or `/tree` navigation back to that origin is a separate lifecycle recovery event that re-arms eligible Auto delivery. An automatic wake consumes eligible `Auto continue` results in one hidden message; eligible `Next natural turn` results remain pending until a natural parent prompt. Exact AgentStatus reads join the current parent turn's successful-settlement acknowledgement.
...
- The list and folded Footer stay hidden while no subagent records or active delivery state exist. Normal in-flight Auto delivery adds no pending UI text; a blocked or failed Auto delivery shows nonzero `results pending`, while Next-turn mode shows nonzero `results waiting for next turn`. These states can keep sticky Main visible after a volatile Agent record is cleaned; results from unrelated branches remain hidden. The list defaults to expanded for each extension runtime, and `Alt+A` gives the user sole control over folding thereafter; transient zero records, pinning, and lifecycle changes do not alter that choice. While expanded, sticky Main owns the orange needs-input count, explicit running/queued/total counts, `Alt+A collapse`, and active-child `Alt+M main`, followed by at most six subagents; this extension emits no Footer status. While folded, the Footer becomes the compact replacement, uses `Subagent`/`Subagents`, and shows the same summary with `Alt+A expand`. Both forms follow needs input, counts, delivery state when present, `Alt+A`, then `Alt+M`, allowing Pi to discard trailing help first on narrow screens. Child-interaction failures replace the expanded Main or folded Footer summary with a local `Blocked:` reason; they do not notify in Main's transcript area.
- Persisted terminal subagents are normally removed from the volatile list after 10 minutes; their parent-session result entries remain available for origin-subtree delivery and explicit session-wide AgentStatus lookup. An explicit result read is acknowledged only after its parent turn settles successfully. New/forked sessions ignore copied entries whose parent session ID does not match. Space toggles independent session-local pins on highlighted subagents; pins pause automatic cleanup, do not change status ordering, and never block explicit Ctrl+D removal. Unpinning resumes the prior remaining duration rather than granting a fresh window.
...
- Recoverable-failure presentation is local-only. Parent-session result entries are the only durable handoff; external transports such as webhooks, Telegram, and email are deferred until a concrete consumer exists. Future notifications must not include prompts, transcripts, source code, or findings by default.
