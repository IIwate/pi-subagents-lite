...
A terminal background result from a session without human takeover is persisted with the parent session ID and Agent-call origin entry before one automatic wake opportunity. Concurrent wake requests are coalesced. A completion persisted during a failed parent turn provides one later wake opportunity after settlement; the failed delivery alone never retries itself. A later persisted completion can carry older eligible pending results. Automatic delivery is allowed only while the origin entry remains on the active branch. Explicit session reload or `/tree` navigation back to that origin is a separate restoration event. The next natural parent prompt injects eligible pending results during preflight, including after a failed automatic wake. Result ACK confirms durable ingestion into the parent session log, independent of LLM turn success or error. Exact AgentStatus reads join durable receipt reconciliation.
_Avoid_: Session-global injection, 200ms debounce, logical task batch, join mode

**Human takeover**:
`Alt+T` explicitly takes over the highlighted or active subagent, pins its record, and detaches any foreground wait. Ordinary input queues steering and preserves autonomous delivery; the host FollowUp action uses the separate follow-up queue. Subsequent output after takeover stays in the child session for explicit message selection; completion itself neither writes a parent inbox entry nor requests a wake. Each explicit selection creates a separate persisted delivery without changing prior entries. The editor owns instructions and continuation; the delivery selector owns message selection. The current AgentManager composition keeps child sessions and unselected output in memory; the native TaskEngine adapter has its own persisted execution contract.
