...
**Accepted run policy**:
The deep-copied Agent definition, resolved tool/skill/extension loading policy, system prompt mode, context-file setting, model, parent model, thinking selection, scoped-model state, and grace turns locked after an Agent call passes authorization. Running and queued agents use this snapshot; later registry, configuration, parent-model, or scope changes affect only future calls. Prompt text and resources are prepared before the task is published to the native queue.
_Avoid_: Queue revalidation, live assignment

**Background result delivery**:
An autonomous background completion is saved to the native outbox with stable task, operation, delivery, and parent-origin identities. The parent adapter waits for an idle eligible branch, writes the result, and verifies its durable receipt before acknowledging it. Parent lifecycle changes and explicit reload provide retry opportunities. Exact AgentStatus reads can also form durable receipts; model success, rendering, or an in-memory message is insufficient.
_Avoid_: Session-global injection, 200ms debounce, logical task batch, join mode

**Human takeover**:
`Alt+T` explicitly takes over the highlighted or active subagent, pins its record, and detaches any foreground wait. Ordinary input queues steering and preserves autonomous delivery; the host FollowUp action uses the separate follow-up queue. Subsequent output after takeover stays in the child session for explicit message selection; completion itself neither creates an automatic outbox entry nor requests a wake. Each explicit selection creates a separate persisted delivery without changing prior entries. The editor owns instructions and continuation; the delivery selector owns message selection. The native session durably retains the transcript and control mode across reload.

A user stop applies to the current operation and preserves the task's control mode. Its retained output is available through `Alt+S` after settlement, and its automatic delivery is suppressed. Later operations follow the task's existing control mode.
...
- The `Agent` tool is the only spawn entry point. `/agents` configures model access, concurrency, execution defaults, prompts, and display. Revisit only if users need to start agents without involving the parent LLM.
...
- Foreground Agent calls are bound to Pi's parent tool-call interrupt signal. Interrupting a parent turn stops its running foreground Agents and removes its queued foreground Agents from execution while retaining `Stopped` records; background Agents remain detached. While a child view is active, `Esc` stops its active operation. In other list states, `Esc` returns focus to the editor.
- The list and folded Footer stay hidden while no subagent records or active delivery state exist. Normal in-flight automatic delivery adds no pending UI text; a blocked or failed delivery shows nonzero `results pending`. This state can keep sticky Main visible after a settled task is hidden; results from unrelated branches remain hidden. The persisted Display setting defaults new conversations to an expanded list, while `Alt+A` gives the user sole control over folding for the current runtime thereafter; changing the default does not override the current choice, and the menu tells the user that `/reload` recreates the runtime and applies the new default immediately. Transient zero records, pinning, and lifecycle changes do not alter the current choice. While expanded, sticky Main owns explicit running/queued/total counts, `Alt+A collapse`, and active-child `Alt+M main`, followed by at most six subagents; this extension emits no Footer status. While folded, the Footer becomes the compact replacement, uses `Subagent`/`Subagents`, and shows the same summary with `Alt+A expand`. Child-interaction failures replace the expanded Main or folded Footer summary with a local `Blocked:` reason; they do not notify in Main's transcript area.
- Settled subagents leave the display after 10 minutes; native results and outbox entries remain available for eligible origin-branch delivery and explicit session-wide AgentStatus lookup. A result read is acknowledged only after its receipt persists in the parent log. Discovery is scoped by parent session ID. Space toggles pins, which pause the remaining display window and do not block explicit removal. Unpinning resumes that remaining window.
- An autonomous background failure follows ordinary `Error` result persistence and parent delivery. A retained settled native task accepts subsequent prompts through its editor, including recovery after an error or an interrupted foreground run, and preserves its control mode. After explicit **Human takeover**, selection controls which output enters the native outbox. The parent LLM has no continuation tool. See the [delivery Note](.agents/notes/implemented/architecture/2026-09-11-native-execution-and-parent-delivery-adapters.md) for persistence and receipt semantics.
...
- Error presentation is local-only. Native outbox entries and verified parent receipts own durable delivery; external transports such as webhooks, Telegram, and email are deferred until a concrete consumer exists. Future notifications must not include prompts, transcripts, source code, or findings by default.
...
Contributor commands, test execution, and worktree setup are documented in the [development guide](docs/development.md).
