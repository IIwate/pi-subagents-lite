...
Once a subagent exists, progress appears in the below-editor list or its folded Footer summary. The list has a sticky Main row and up to six visible subagents scrolled around the focused row. While Main is active and the list is not keyboard-focused, the viewport follows the first Running row; an all-terminal list starts at the top. It starts expanded by default; `/agents` → Display settings → Expand list by default persists a different initial choice for new conversations. After changing it, the menu reports that `/reload` applies the new default immediately by recreating the current conversation's extension runtime. `Alt+A` toggles only the current runtime, and that choice remains even if the volatile record count temporarily reaches zero. With no records and no pending results eligible for the active branch, both the list and footer status stay hidden. Status follows the agent name in parentheses; provider, model, and thinking appear before usage stats. Agent rows use status-rank order: attention (Error, Aborted, Turn limit), then Running, then Queued, then archived Done and Stopped. Within each rank, pinned rows come first; terminal rows show the newest completion first, while Running and Queued rows keep start/FIFO order:
...
- Pins pause automatic cleanup and lift a row only within its current status rank. Multiple Agents may be pinned; unpinning restores the rank's time order and resumes the remaining cleanup time rather than granting a fresh window.
...
- Foreground Agent calls honor Pi's interrupt signal from the start of session setup: `Esc` from the editor stops every running or queued foreground Agent in the interrupted parent turn before it can send a late first prompt, while background Agents continue. If the list has focus, `Esc` only returns to the editor; press it again there to interrupt.
- While a subagent is active, editor input is routed to that session. Its current streaming message refreshes once per second without copying every other session or the finalized transcript history. Press `Alt+M` to return to Main from either an expanded or folded list; this changes only the active transcript and input route, not list visibility or child execution. Switching Pi between regular and fullscreen TUI modes preserves the selected child, transcript, and input route because both renderers reuse the same component tree. The built-in Main cwd and model-usage footer rows are hidden on the child screen, leaving extension statuses; a custom footer supplied by another extension is preserved, including replacements made while the child is active. If Pi exposes an unsupported host layout, child activation fails closed with one warning instead of partially replacing Main.
- Persisted terminal results are normally removed from the volatile Agent list after 10 minutes; the parent session result entry remains available for later delivery and exact lookup. Each explicit `AgentStatus({ agent_id })` lookup rereads the current parent-session entries, so a result written by a replacement runtime is not hidden by an older in-memory continuation. Automatic delivery is limited to branches that retain the Agent call's origin entry; explicit lookup remains session-wide. An `Error` record with a retained live session can accept another prompt through the selected child view during the same ordinary retention period. That interaction produces a new terminal result and never retracts or duplicates the first delivery. Pinning extends ordinary retention; viewing does not. Child sessions and pins are not persisted across `/reload` or process exit, and the parent LLM has no continuation tool.
...
Persisted finite numeric limits are normalized on read to preserve older scheduler behavior: values below 1 become 1 and positive fractions round up. Missing, non-numeric, `NaN`, and infinite values still fall back or are ignored. Settings continue to write only integers of at least 1.
...
Manual migration is needed only if all of these are true: the configuration was written by an older release, Pi was launched on Windows without `HOME`, and the files actually exist under the launch or project directory instead of the OS home. Other users are unaffected.
