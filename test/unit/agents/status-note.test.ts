...
import { getStatusNote } from "../../../src/agents/status-note.js";

describe("getStatusNote", () => {
  it("returns empty string for status without a note", () => {
    expect(getStatusNote({ status: "completed", result: "" })).toBe("");
  });

  it("returns user stop message when stoppedBy is user", () => {
    expect(getStatusNote({ status: "stopped", stoppedBy: "user" })).toMatch(/STOPPED BY THE USER/);
  });

  it("returns agent stop message when stoppedBy is agent", () => {
    expect(getStatusNote({ status: "stopped", stoppedBy: "agent" })).toMatch(/stopped before completion/);
  });

  it("returns agent stop message when stoppedBy is undefined", () => {
    expect(getStatusNote({ status: "stopped" })).toMatch(/stopped before completion/);
  });

  it("wraps known notes with space-parentheses", () => {
    expect(getStatusNote({ status: "stopped", stoppedBy: "user" })).toMatch(/^ \(.+\)$/);
  });
  ...
  it("tells the parent a hard abort produced no answer and how to recover", () => {
    ...
    const note = getStatusNote({ status: "aborted" });
  ...
  it("keeps the soft turn limit distinct from a hard abort", () => {
    ...
    const note = getStatusNote({ status: "turn_limited", result: "" });
