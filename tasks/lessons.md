...
- Fix: bracket `runAgent` with a process-level `AsyncLocalStorage` marker and make the factory + `session_start`/`session_shutdown` handlers inert only in that child async chain. A module-local depth counter is lost across Jiti/cwd cache reloads and can also block unrelated parent reloads. `dispose()` gates deferred work after `session_shutdown`.
