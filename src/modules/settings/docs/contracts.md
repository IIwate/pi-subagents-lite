...
- `ConcurrencySettingsView`/`ConcurrencyLimitUpdate` define the concurrency boundary (REQ-CONFIG-003, REQ-RUNTIME-008): the merged effective fragment and provenance for display, the sparse global and project layer fragments that drive the selected target's row set, the `ConcurrencyProjectLayerView` (state/filePath/writable/ignoredEntryCount), and the active provider/model inventory. `ConcurrencyLimitUpdateSchema` wraps the runtime's `ConcurrencyLimitsUpdateSchema` with a `ConcurrencyTarget` so every write names exactly one layer. `ProjectLayerStateSchema` is the settings-owned page vocabulary: `untrusted` plus configuration's document status; configuration itself never owns the trust word.
