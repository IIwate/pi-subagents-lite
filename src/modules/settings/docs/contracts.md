...
- `ConcurrencySettingsView`/`ConcurrencyLimitUpdate` define the concurrency boundary (REQ-CONFIG-003, REQ-RUNTIME-008): the merged effective fragment and provenance for display, the sparse global and project layer fragments that drive the selected target's row set, the `ConcurrencyProjectLayerView` (state/filePath/writable/ignoredEntryCount), and the active provider/model inventory. `ConcurrencyLimitUpdateSchema` wraps the runtime's `ConcurrencyLimitsUpdateSchema` with a `ConcurrencyTarget` so every write names exactly one layer. The fallback-default row follows the same rule as keyed overrides: a plain numeric input while the selected layer names no explicit default, an edit-or-remove `limit` row once it does — removal restores inheritance on the project layer and the factory default on the global layer. `ProjectLayerStateSchema` is the settings-owned page vocabulary: `untrusted` plus configuration's document status; configuration itself never owns the trust word.
...
- `ConcurrencySettingsOwner` reads the concurrency view and commits limit updates; the fragment's shape and runtime effect belong to the subagent-runtime module, and the live scheduler is republished only after a successful commit. The write target is session-local page state: entering the page starts on Global, switching targets performs no IO, and the merged effective values are never a write set. Clearing an override the selected layer never had is a no-op success that creates no file; a `document-malformed` refusal surfaces as an explicit notice with the previous value kept.
