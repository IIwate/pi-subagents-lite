...
- [test/support](../../../../test/support/) contains configuration, catalogue instances, resource disposal, and focused module fixtures. `createTestHarness` owns temporary directories, asynchronous disposal, mocks, and clocks. Temporary directories use native physical paths so aliases and Windows short names agree with production filesystem consumers. Runtime scenarios share an [official parent-session fixture](../../../../test/support/runtime.ts) with offline providers and native session files. The delivery adapter verifies receipts in the real parent log; memory-only publication is not an acknowledgement.
- Offline provider fixtures supply responses and failures to the real Pi agent loop; rendering tests supply navigation snapshots. Scheduling, retries, and delivery follow the production paths. Fixture gates control when a provider response becomes available and release during teardown. [Runtime scenarios](../../../../test/scenarios/runtime.test.ts) verify lifecycle ownership, failure, explicit continuation, and durable output. [Child-resource scenarios](../../../../test/scenarios/runtime-resources.test.ts) verify accepted working directories, project trust, prompts, tools, and extension hooks.
