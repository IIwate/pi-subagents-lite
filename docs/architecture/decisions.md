...
## Global/project configuration layering

The persisted configuration exists in two independent documents: the global file in the Pi agent directory and, for a trusted project, `<cwd>/<config-dir>/subagents-lite.json`. The project document is an override layer, never a replacement — absence of a key means inheritance, there are no tombstones, and the merged effective values are never written back to any file. The binding is to the parent session's cwd and does not follow `worktree_path` spawns. Trust gates the layer exactly as it gates project Agent definitions (see [Project resource trust](#project-resource-trust)): an untrusted session gets a bare binding with no file path and no IO, so the file is never read.

Ownership: `bootstrap/configuration.ts` keeps a process-scoped Map from the lexically normalized config file path (`normalizeConfigPathKey`: forward slashes, Windows-style paths lowercased, no realpath) to one configuration facade + section IO — the same "one document, one process-scoped owner" decision as the global constant. Two runtimes on the same project therefore share one revision domain and cannot lose each other's updates; different projects share nothing; global and each project document are independent revision domains. Symlink/junction aliases may produce distinct keys — an accepted boundary, since realpath on possibly absent files buys complexity without a concrete need. Project facades use the `read-only` malformed policy: a malformed project file is excluded from the effective layer, offers no write target, and is never overwritten or auto-repaired. Rejected alternatives: an owner Map on `globalThis`/process-state (violates its two-responsibility whitelist), a shared shell revival, and per-runtime facades (independent revisions → lost updates).

Future Model access project policy is recorded, not implemented: if introduced, its semantics must be `effective authorization = global authorization ∩ trusted project restriction ∩ Pi availability/scope` — a project can only narrow, never enable, and it requires its own PRD, schema, UI, and test task.
